...
class BatchProcessor:
    ...
    def process(self, video_paths: List[str]) -> List[Dict[str, Any]]:
        ...
        """
        # Resolve patterns and collect all video files
    ...
    def _process_multi_video(self, video_files: List[str]) -> List[Dict[str, Any]]:
        ...
        # Phase 1: Extract clips from each video
        for i, video_path in enumerate(video_files):
            ...
            # Collect clips with valid paths
            for clip in clips:
        ...
        # Phase 2: Merge all clips into one video
        if not all_clips:
        ...
        # Mix audio (add background music if configured)
        mixer = AudioMixer(self.config)
        result_path = mixer.mix_audio(merged_no_audio, final_output)
        
        # If mixer skipped (no music), copy the merged file
        if result_path == merged_no_audio:
            import shutil
            shutil.copy2(merged_no_audio, final_output)
        
        # Clean up temp file
        if os.path.exists(merged_no_audio) and merged_no_audio != final_output:
            try:
